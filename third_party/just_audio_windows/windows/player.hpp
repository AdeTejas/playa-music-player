...
// Forward declaration for helper implemented in the plugin registration file.
...
public:
  ...
  void Success(const EncodableValue& event) {
    if (sink) {
      // Encode the event envelope now and post it to the platform thread to
      // avoid sending messages from non-platform threads.
      auto payload = flutter::StandardMethodCodec::GetInstance().EncodeSuccessEnvelope(&event);
      if (payload) {
        JustAudio_PostEncodedEnvelopeToPlatformThread(messenger_, channel_name_, std::move(payload));
      }
    }
  }
  ...
  void Error(const std::string& error_code,
    ...
    if (sink) {
      auto payload = flutter::StandardMethodCodec::GetInstance().EncodeErrorEnvelope(error_code, error_message, nullptr);
      if (payload) {
        JustAudio_PostEncodedEnvelopeToPlatformThread(messenger_, channel_name_, std::move(payload));
      }
    }
...
private:
  ...
  // Keep the messenger and channel name so we can post encoded envelopes
  // to the platform thread without referencing the EventSink on non-platform
  // threads.
  const flutter::BinaryMessenger* messenger_ = nullptr;
...
public:
  ...
  AudioPlayer::AudioPlayer(std::string idx, flutter::BinaryMessenger* messenger) {
    ...
    std::cerr << "[just_audio_windows] Registered player channel: com.ryanheise.just_audio.methods." << id << " ts=" << ms << std::endl;
    ...
    std::cerr << "[just_audio_windows] Event channels: events." << idx << " data." << idx << " ts=" << ms << std::endl;
  ...
  AudioPlayer::~AudioPlayer() {
    auto now = std::chrono::system_clock::now();
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
    std::cerr << "[just_audio_windows] ~AudioPlayer id=" << id << " ts=" << ms << std::endl;
  ...
  void AudioPlayer::HandleMethodCall(
    ...
    std::cerr << "[just_audio_windows] player id=" << id << " Called " << method_call.method_name() << std::endl;

    if (disposing) {
      return result->Error("player_disposing", "Player is being disposed");
    }
  ...
  */
  ...
  void AudioPlayer::broadcastPlaybackEvent() {
    ...
    try {
      ...
      std::cerr << "[just_audio_windows]: Broadcast playback event error: BufferingProgress access failed: " << winrt::to_string(ex.message()) << std::endl;
      bufferingProgress = 1.0;
    } catch (...) {
      std::cerr << "[just_audio_windows]: Broadcast playback event error: Unknown exception accessing BufferingProgress. Using default 1." << std::endl;
      bufferingProgress = 1.0;
    }
  ...
  void AudioPlayer::broadcastDataEvent() {
    ...
    // Guard accesses that can throw
    try {
      eventData[flutter::EncodableValue("volume")] = flutter::EncodableValue(mediaPlayer.Volume());
    } catch (winrt::hresult_error const& ex) {
      std::cerr << "[just_audio_windows]: Broadcast data event error: Volume access failed: " << winrt::to_string(ex.message()) << std::endl;
      eventData[flutter::EncodableValue("volume")] = flutter::EncodableValue(1.0);
    } catch (...) {
      eventData[flutter::EncodableValue("volume")] = flutter::EncodableValue(1.0);
    }
    ...
    try {
      ...
      std::cerr << "[just_audio_windows]: Broadcast data event error: PlaybackRate access failed: " << winrt::to_string(ex.message()) << std::endl;
      eventData[flutter::EncodableValue("speed")] = flutter::EncodableValue(1.0);
    } catch (...) {
      eventData[flutter::EncodableValue("speed")] = flutter::EncodableValue(1.0);
    }

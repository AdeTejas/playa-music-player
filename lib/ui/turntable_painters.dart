...
class TurntableBasePainter extends CustomPainter {
  ...
  TurntableBasePainter({required this.strobeEnabled, required this.strobeColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Wood Texture - Enhanced
    final plinthPaint = Paint()
      ..shader = ui.Gradient.linear(
        plinthRect.topLeft,
        plinthRect.bottomRight,
        [const Color(0xFF8D5524), const Color(0xFF5D3A1A), const Color(0xFF3E2712)], // Richer Walnut with depth
        [0.0, 0.6, 1.0],
      );
    ...
    // Add subtle noise/speckle for realism
    ...
    // Mesh Pattern
    canvas.save();
    canvas.clipRRect(RRect.fromRectAndRadius(speakerRect.deflate(w * 0.01), Radius.circular(w * 0.01)));
    ...
    for (double dx = speakerRect.left; dx < speakerRect.right; dx += dotSpacing) {
      for (double dy = speakerRect.top; dy < speakerRect.bottom; dy += dotSpacing) {
        // Circular pattern mask
        if ((Offset(dx, dy) - speakerRect.center).distance < speakerSize * 0.42) {
    ...
    // Labels "33" and "45"
    canvas.drawCircle(rpmBaseRect.centerLeft.translate(w * 0.02, 0), 2, Paint()..color = const Color(0xFF555555));
    canvas.drawCircle(rpmBaseRect.centerRight.translate(-w * 0.02, 0), 2, Paint()..color = const Color(0xFF555555));
...
class TurntableSpinnerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const labelColor = Color(0xFFDDDDDD); // Silver/Grey
    ...
    canvas.drawLine(
      ...
      Offset(w * 0.02, w * 0.015),
      Paint()..color = mcrnOrange..strokeWidth = w * 0.005
    );
